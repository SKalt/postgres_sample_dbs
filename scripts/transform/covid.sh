...
. "$this_dir/../common.sh"
source_dir="$repo_root/tmp/yugabyte-db/sample"
...
main() {
  ...
  {
    unzip -d "$intermidiate_dir" "$source_dir/covid-data-case-study/covid-data-case-study.zip"
  } | log_debug
  ...
  log_info "running database"
  {
    docker_compose exec pg sh -c '
      ...
      psql --set=ON_ERROR_STOP=on -f ./0.sql # --set=ON_ERROR_STOP=on
      '
  } | log_debug
